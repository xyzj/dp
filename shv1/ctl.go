...
func (dp *DataProcessor) ParseCtl(b []byte) (lstf []*Fwd) {
		...
		for _,v:=range pb2data.DataID.UintID {
			...
			case 0:
				...
				case 3: // 终端事件记录配置设置
					...
					d.Write([]byte{gopsu.String2Int8(s1[0:8], 2),gopsu.String2Int8(s1[8:16], 2),gopsu.String2Int8(s1[16:32], 2),gopsu.String2Int8(s1[32:40], 2),gopsu.String2Int8(s1[40:48], 2),gopsu.String2Int8(s1[48:56], 2),gopsu.String2Int8(s1[56:64], 2)})
				...
				case 9: // 控制器开关灯时间参数
					d.Write(setPnFn(v.Pn))
					d.Write(setPnFn(v.Fn))
					d.Write([]byte{gopsu.String2Int8(pb2data.Afn04P0F9.DtStart[0:2], 2),gopsu.String2Int8(pb2data.Afn04P0F9.DtStart[2:4], 2)})	
				...
				case 12: // 控制回路参数
					...
					s1 := gopsu.ReverseString(strings.Join(rs, ""))
					d.Write([]byte{gopsu.String2Int8(s1[0:8], 2),gopsu.String2Int8(s1[8:16], 2)})
		...
		for _,v:=range pb2data.DataID.UintID {
			...
			case 0:
				...
				case 11: // 模拟量批量查询
					...
					d.WriteByte(byte(len(pb2data.Afn0CP0F11.LoopData)))	
				
				case 12: // 状态量批量查询
					d.Write(setPnFn(v.Pn))
					d.Write(setPnFn(v.Fn))	
					d.WriteByte(byte(pb2data.Afn0CP0F12.LoopStart))	
					d.WriteByte(byte(len(pb2data.Afn0CP0F12.LoopStatus)))	
				}
			}
		}
	case 0x0e: // 请求事件数据
	...
	ff := &Fwd{
		...
		DataDst:  fmt.Sprintf("shv1-rtu-%d", pb2data.DataID.Addr),
		DstType:  SockTml,
